<?php
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST")
	{
		...
		$sql = "SELECT balance FROM users where user_id = '$top_up_id'";
		$top_up_account = mysqli_query($link, $sql);
		
		if (mysqli_num_rows($top_up_account) > 0)
		{

			while($row = mysqli_fetch_array($top_up_account))
			{
					
					$new_balance = $row["balance"] + $top_up_amount;
					
			}

			$sql = "UPDATE users SET balance = '$new_balance' WHERE user_id = '$top_up_id'";
			mysqli_query($link, $sql);
			
			$issued_by = $_SESSION["user_id"];
			
			$sql = "INSERT INTO transactions (user_id, amount, description, issued_by) VALUES ('$top_up_id', '$top_up_amount', 'Fund Transfer', '$issued_by')";
			mysqli_query($link, $sql);
        ...
        <div class="container">
            <div class="row mb-5">
                <div class="col-md-8 col-xl-6 text-center mx-auto">
                    <h1 style="color: #f3f3f2;font-weight: bold;">Top Up</h1>
                    ...
                    <div class="card mb-5" style="background: #f3f3f2;">
                        <div class="card-body d-flex flex-column align-items-center">
                            ...
                            <form class="text-center" method="post">
                                <div class="mb-3"><input class="form-control form-control-lg" type="text" name="top_up_id" required="" placeholder="ID Number" style="text-align: center;" autofocus="" autocomplete="off"></div>
                                <div class="mb-3"><input class="form-control form-control-lg" type="number" required="" name="top_up_amount" placeholder="Amount (₱)" style="text-align: center;"></div>
                                <div class="mb-3"><button class="btn btn-primary btn-lg d-block w-100" type="submit" style="background: #10ae4d;border-style: none;">Top Up</button></div>
                            </form>
                        </div>
                    </div>
                </div>
