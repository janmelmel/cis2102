...
<body style="background: #f3f3f2;">
	...
	<section style="background: #164428;padding-top: 40px;padding-bottom: 40px;">
		<div class="container">
			<div class="card" style="background: #f3f3f2;">
				<div class="card-body">
					<div class="row">
						<div class="col-12 col-sm-12 col-md-12 col-lg-6 col-xl-6 col-xxl-6" style="height: 500px;">
							<h3 style="text-align: center;">To Claim</h3>
							<hr />
							<div class="overflow-scroll" style="height: 400px;overflow-x: hidden !important;">
								<?php
								
									require_once "config.php";
								
									$sql = 'SELECT book_code, timestamp_created, status from library_transactions WHERE (status = 0 || status = 1) && user_id = "' . $_SESSION["user_id"] . '"';
									$for_claiming = mysqli_query($link, $sql);
									
									while($row = mysqli_fetch_array($for_claiming))
									{
									
										$status_color = "rgba(255,193,7,0.25)";
										
										if ($row["status"] == 1)
										{
										
											$status_color = "rgba(25,135,84,0.25)";
										
										}
									
										$sql = 'SELECT title FROM library_items WHERE book_code = "' . $row["book_code"] . '"';
										$book_title = mysqli_query($link, $sql);
										
										while ($row2 =mysqli_fetch_array($book_title))
										{
										
											echo '<div class="card card-hover" style="border-radius: 0px;">
											<div class="card card-hover" style="border-radius: 0px; background:' . $status_color . ';">
												<div class="card-body">
												<h4 class="text-truncate card-title">' . $row2["title"] . '</h4>
										<h6 class="text-muted card-subtitle mb-2">' . $row["book_code"] . '</h6>
										<h6 class="text-muted card-subtitle mb-2">' . $row["timestamp_created"] . '</h6>
										</div>
									</div>
								</div>';
						...
						<div class="col-12 col-sm-12 col-md-12 col-lg-6 col-xl-6 col-xxl-6" style="height: 500px;">
							<h3 style="text-align: center;">Currently Borrowing</h3>
							<hr />
							<div class="overflow-scroll" style="height: 400px;overflow-x: hidden !important;">
								<?php
									...
									$sql = 'SELECT book_code, timestamp_created FROM library_transactions WHERE user_id = "' . $_SESSION["user_id"] . '" && is_borrow = 1 && status != 3';
									$currently_borrowing = mysqli_query($link, $sql);
									
									while ($row = mysqli_fetch_array($currently_borrowing))
									{
										...
										while ($row2 = mysqli_fetch_array($title))
										{
											...
											echo '<div class="card card-hover" style="border-radius: 0px;">
									<div class="card card-hover" style="border-radius: 0px;">
										<div class="card-body">
											<h4 class="text-truncate card-title">' . $row2["title"] . '</h4>
											<h6 class="text-muted card-subtitle mb-2">' . $row["book_code"] . '</h6>
											<h6 class="text-muted card-subtitle mb-2">' . $row["timestamp_created"] . '</h6>
	...
	<section style="background: #164428;padding-bottom: 40px;">
		<div class="container">
			<div class="card" style="background: #f3f3f2;">
				<div class="card-body">
					<div class="row">
						<div class="col-12 col-sm-12 col-md-12 col-lg-6 col-xl-6 col-xxl-6" style="height: 500px;">
							<h3 style="text-align: center;">Borrow History</h3>
							<hr />
							<div class="overflow-scroll" style="height: 400px;overflow-x: hidden !important;">
								<?php
								
									$sql = 'SELECT book_code, timestamp_created, timestamp_returned FROM library_transactions WHERE user_id = "' . $_SESSION["user_id"] . '" && is_borrow = 1 && status = 3';
									$borrow_history = mysqli_query($link, $sql);
									
									while($row = mysqli_fetch_array($borrow_history))
									{
									...
									<div class="card card-hover" style="border-radius: 0px;">
										...
										<h4 class="text-truncate card-title">' . $row2["title"] . '</h4>
										<h6 class="text-muted card-subtitle mb-2">' . $row["timestamp_created"] . '</h6>
										<h6 class="text-muted card-subtitle mb-2">' . $row["timestamp_returned"] . '</h6>
										</div>
									</div>
								</div>	';
						...
						<div class="col-12 col-sm-12 col-md-12 col-lg-6 col-xl-6 col-xxl-6" style="height: 500px;">
							...
							<div class="overflow-scroll" style="height: 400px;overflow-x: hidden !important;">	
								<?php
									...
									$sql = 'SELECT book_code, timestamp_created, timestamp_released from library_transactions WHERE status < 3 && is_borrow = 0 && user_id = "' . $_SESSION["user_id"] . '"';
									$purchase_history = mysqli_query($link, $sql);
									
									while($row = mysqli_fetch_array($purchase_history))
									{
											
										$timestamp_released_code = "";
											
										if ($row["timestamp_released"] !== NULL)
										{
											
											$timestamp_released_code = '
										<h6 class="text-muted card-subtitle mb-2">' . $row["timestamp_released"] . '</h6>';
											
										}
											
										$transaction_description = 'Purchase of ' . $row["book_code"];
											
										$sql = 'SELECT amount FROM transactions WHERE description = "' . $transaction_description . '"';
										$purchase_amount = mysqli_query($link, $sql);
											
										while ($row2 = mysqli_fetch_array($purchase_amount))
										{
											
											$amount = number_format(($row2["amount"] * -1), 2, '.', ',');
									...
									<div class="card card-hover" style="border-radius: 0px;">
										<div class="card-body">
											<h4 class="text-truncate card-title">' . $title . '</h4>
											<h6 class="text-muted card-subtitle mb-2">₱' . $amount . '</h6>
											<h6 class="text-muted card-subtitle mb-2">' . $row["timestamp_created"] . '</h6>' . $timestamp_released_code . '
