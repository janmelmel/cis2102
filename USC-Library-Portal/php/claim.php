<?php
	...
	if(isset($_POST['mark_as_ready']))
	{
		...
		$sql = 'UPDATE library_transactions SET status = 1 WHERE book_code = "' . $book_code . '" ORDER BY timestamp_created DESC LIMIT 1';
		mysqli_query($link, $sql);
		
	}
	
	if(isset($_POST['unmark_as_ready']))
	{
		...
		$sql = 'UPDATE library_transactions SET status = 0 WHERE book_code = "' . $book_code . '" ORDER BY timestamp_created DESC LIMIT 1';
		mysqli_query($link, $sql);
	...
	if(isset($_POST['claim']))
	{
		...
		$book_code = $_POST["claim_book_code"];
		$notes = $_POST["claim_notes"];
		
		$sql = 'UPDATE library_transactions SET status = 2, claim_notes = "' . $notes . '", timestamp_released = CURRENT_TIMESTAMP, released_by = "' . $_SESSION["user_id"] . '" WHERE user_id = "' . $claim_id . '" && book_code = "' . $book_code . '" ORDER BY timestamp_created DESC LIMIT 1';
		$claim = mysqli_query($link, $sql);
...
<body style="background: #f3f3f2;">
        ...
        <div class="container">
            <div class="row mb-5">
                <div class="col-md-8 col-xl-6 text-center mx-auto">
                    <h1 style="color: #f3f3f2;font-weight: bold;">Claim</h1>
                    ...
                    <div class="card mb-5" style="background: #f3f3f2;">
                        <div class="card-body d-flex flex-column align-items-center">
                            ...
                            <form class="text-center" method="post">
                                <div class="mb-3"><input class="form-control form-control-lg" type="text" name="claim_id" required="" placeholder="ID Number" style="text-align: center;" autocomplete="off"></div>
                                <div class="mb-3"><input class="form-control form-control-lg" type="text" name="claim_book_code" required="" placeholder="Book Code" style="text-align: center;" autocomplete="off"></div>
                                <div class="mb-3"><textarea class="form-control form-control-lg" name="claim_notes" placeholder="Notes" autocomplete="off"></textarea></div>
                                <div class="mb-3"><button class="btn btn-primary btn-lg d-block w-100" type="submit" name="claim" style="background: #10ae4d;border-style: none;">Claim</button></div>
                            </form>
                        </div>
                    </div>
                </div>
	...
	<section style="background: #164428;padding-bottom: 48px;">
		<div class="container">
			<div class="card">
				<div class="card-body">
					<h1 style="text-align: center;">For Claiming</h1>
					<hr />
					<div>
						...
						<?php
							...
							$sql = 'SELECT timestamp_created, book_code, status FROM library_transactions WHERE status < 2';
							...
							while ($row = mysqli_fetch_array($for_claiming))
							{
								...
								if ($row["status"] == 0)
								{
									...
									$background_status_color = 'rgba(255,193,7,0.25)';
									$button = '<button class="btn btn-primary" type="submit" name="mark_as_ready" style="background: #10ae4d;border-style: none;">Mark as Ready</button>';
								
								}
								elseif ($row["status"] == 1)
								{
								
									$background_status_color = 'rgba(25,135,84,0.25)';
									$button = '<button class="btn btn-primary" type="submit" name="unmark_as_ready" style="background: #10ae4d;border-style: none;">Unmark as Ready</button>';
								...
								while ($row2 = mysqli_fetch_array($title))
								{
									...
									echo '<div class="card card-hover" style="border-radius: 0px;background: ' . $background_status_color . ';">
    						<div class="card-body" style="text-align: center;">
        						<h4 class="text-truncate card-title">' . $row2["title"] . '</h4>
								<h5 class="text-muted card-subtitle mb-2">' . $row["book_code"] . '</h5>
        						<h5 class="text-muted card-subtitle mb-2">' . $row["timestamp_created"] . '</h5>
        						<form method="post"><input class="form-control" type="hidden" name="book_code" value="' . $row["book_code"] . '"/>' . $button . '</form>
    						</div>
						</div>';
